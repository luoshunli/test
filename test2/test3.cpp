...
struct Node { 
	...
	Q _q;
	Node *_next; 
	Node(double tc, Q q, Node *next):_tc(tc), _q(q), _next(next) {} 
}; 
...
void* func1(void *arg) { 
	while (1) { 
		...
		pthread_cond_wait(&cond, &mtx); 
			...
			Node *p = head; 
			head = p->_next; 
			std::cout << p->_tc <<" " << (p->_q).x <<" " << (p->_q).y <<" " << (p->_q).z <<" " << (p->_q).y <<endl; 
			delete p; 
			p = NULL; 
		} 
...
void* func2(void *arg) { 
	...
	while (1) { 
		pthread_mutex_lock(&mtx); 
		if(CalImuPose(cam_t[i], imu_t, imu_q, qc)) {
			Node* p = new Node(cam_t[i], qc, head);
			head = p; 
		}
		pthread_mutex_unlock(&mtx); 
